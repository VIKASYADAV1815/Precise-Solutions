...
export default function AluminiumDoorFittingPage() {
  const [currentSlide, setCurrentSlide] = useState(0);

  const features = [
    { name: "Premium Materials", desc: "Crafted with high-grade, corrosion-resistant metals that offer exceptional durability and longevity." },
    { name: "Precision Engineering", desc: "Engineered for flawless operation, ensuring smooth and reliable performance over years of use." },
    { name: "Modern Aesthetics", desc: "Sleek lines and contemporary finishes that effortlessly elevate any architectural space." },
    { name: "Easy Installation", desc: "Designed with installers in mind, featuring intuitive mechanisms for secure and straightforward mounting." },
    { name: "High Load Capacity", desc: "Built to support significant weight safely, exceeding standard structural requirements." },
    { name: "Low Maintenance", desc: "Treated surfaces that protect against daily wear, requiring minimal upkeep to maintain a pristine look." },
  ];

  const sliderImages = [66, 67];
  const galleryImages = [68, 69];
        ...
        <div className="bg-white rounded-2xl p-6 lg:p-10 shadow-sm border border-slate-200 mb-10 flex flex-col lg:flex-row gap-10 lg:gap-16 items-center">
          ...
          <div className="w-full lg:w-1/2 relative h-[450px] lg:h-[550px] bg-slate-50 rounded-xl flex items-center justify-center border border-slate-100 overflow-hidden">
            <div className="absolute top-4 left-4 bg-white/90 backdrop-blur-sm px-3 py-1.5 rounded-full shadow-sm text-xs font-bold text-slate-600 flex items-center gap-1.5 z-10">
              <Briefcase className="w-3.5 h-3.5" /> Aluminium Fitting
            </div>
            ...
            <div className="relative w-full h-full p-8">
              <Image 
                src={`/lock/aluminium door fitting/Picture${sliderImages[currentSlide]}.png`}
                alt={`Modern Aluminium Door Fittings - View ${currentSlide + 1}`}
                fill
          ...
          <div className="w-full lg:w-1/2 relative z-10">
            ...
            <h1 className="text-3xl lg:text-4xl font-bold text-slate-900 mb-6 tracking-tight leading-tight">
              Modern Aluminium Door Fittings
            </h1>
            
            <p className="text-base text-slate-600 leading-relaxed mb-8">
              Complete your aluminium door installations with our specialized fittings. Designed to integrate perfectly with standard profiles, offering superior durability and a cohesive architectural look.
            </p>
            ...
            <div className="flex items-center gap-6 text-sm font-medium text-slate-600">
              ...
              <div className="flex items-center gap-2"><CheckCircle2 className="w-4 h-4 text-emerald-600" /> High Durability</div>
            </div>
          </div>
        </div>
        ...
        <div className="mb-16 bg-white rounded-2xl p-8 lg:p-12 border border-slate-200 shadow-sm">
          <div className="flex flex-col lg:flex-row gap-12 items-start">
            <div className="w-full">
              ...
              <p className="text-slate-500 mb-8 text-sm">Discover the details that make our hardware collection truly exceptional.</p>
        ...
        <div className="mb-16">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {galleryImages.map((num) => (
              <div key={num} className="relative h-64 sm:h-80 rounded-xl overflow-hidden border border-slate-200 shadow-sm bg-white flex items-center justify-center p-4">
                <Image 
                  src={`/lock/aluminium door fitting/Picture${num}.png`}
                  alt={`Modern Aluminium Door Fittings Setup ${num}`}
                  fill
